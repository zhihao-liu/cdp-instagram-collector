...
function InstaCollector(loginInfo, config) {
...
InstaCollector.prototype.iterateFeed = async function (feed, mongoCollection, limit = Infinity) {
    ...
    try {
      ...
      switch (item.info.mediaType) {
        case 1: { // single image
          ...
          await
            utilities.download(item.info.images[0].url, srcPath)
        ...
        case 2: { // single video
          ...
          const srcPath = path.join(this.config.pathPostMedia, srcName);
          await
            utilities.download(item.info.videos[0].url, srcPath)
              .then(() => srcNames.push(srcName));
        }
          break;
        ...
        case 8: { // multiple images
          for (let i = 1; i < item.info.images.length; ++i) {
            ...
            const srcPath = path.join(this.config.pathPostMedia, srcName);
            await
              utilities.download(item.info.images[i][0].url, srcPath)
                .then(() => srcNames.push(srcName));
          }
        }
          break;
      ...
      await this.mongo.collection('posts')
        .updateOne({'info.id': item.info.id}, {$set: {mediaSrc: srcNames}});
    } catch (err) {
  ...
  while (await cursorCollectionPosts.hasNext()) {
    try {
      ...
      await this.mongo.collection('users')
        .updateOne({'info.id': item.info.id}, {$set: {profileSrc: srcName}});
    } catch (err) {
      InstaCollector.onErrorDownloading(err);
    }
  }
...
InstaCollector.pushIntoMongo = function (array, mongoCollection) {
  let pAllItemsHandled = [];
  for (const item of array) {
    pAllItemsHandled.push(mongoCollection.updateOne({'info.id': item.params.id}, {$set: {info: item.params}}, {upsert: true}));
  }

  return Promise.all(pAllItemsHandled);
};

InstaCollector.prototype.collectUserPosts = async function (option = {historicalFirst: false}) {
  let pAllUsersHandled = [];
  const dateStringToday = utilities.dateStringToday();

  const dateQuery = option.historicalFirst ? {$exists: false} : {$nin: [dateStringToday]};
  const cursorCollectionUsers = this.mongo.collection('users')
    .find({datesFetched: dateQuery, 'info.isPrivate': false});
...
InstaCollector.prototype.startCollectingUsers = async function () {
  ...
  while (true) await this.iterateFeed(userFeed, this.mongo.collection('users'));
};

InstaCollector.prototype.startCollectingPostsNew = async function () {
  while (true) await this.collectUserPosts();
};

InstaCollector.prototype.startCollectingPostsHistorical = async function () {
  while (true) await this.collectUserPosts({historicalFirst: true});
};

InstaCollector.prototype.startCollectingPostsWithHashtag = async function (hashtagString) {
  const postFeed = await new Instagram.Feed.TaggedMedia(this.session, hashtagString);
  while (true) await this.iterateFeed(postFeed, this.mongo.collection(`posts`));
};

InstaCollector.prototype.startCollectingPostsWithLocation = async function (locationString) {
  let location;
  try {
