...
class InstaCollector {
  ...
  connectToMongo() {
    const connectionUrl = `mongodb://localhost:${this.config.mongoPort}/${this.config.mongoName}`;
    ...
    while (await cursorCollectionPosts.hasNext()) {
      try {
        ...
        switch (item.info.mediaType) {
          case 1: { // single image
            const srcPath = path.join(this.config.pathPostMedia, `img_${item.info.id}_0.jpg`);
            await utilities.download(item.info.images[0].url, srcPath)
              .then(() => srcPaths.push(srcPath));
          }
            break;

          case 2: { // single video
            const srcPath = path.join(this.config.pathPostMedia, `vid_${item.info.id}_0.mp4`);
          ...
          case 8: { // multiple images
            for (let i = 1; i < item.info.images.length; ++i) {
              const srcPath = path.join(this.config.pathPostMedia, `img_${item.info.id}_${i}.jpg`);
